...
function setup() {
  ...
  //create space as a spriteObject & add image to space
  space = createSprite(width/2,100,10,10);
  space.addImage(spaceImg);
  space.scale = 2;
  space.velocityY = 5;
  
  //create spaceShip as a spriteObject & add image to spaceShip
  spaceShip = createSprite(width/2,height-35,1,1);
  spaceShip.addImage(spaceShipImg);
  spaceShip.scale = 0.8;
  
  //create gameOver as a spriteObject & add image to gameOver
  gameOver = createSprite(width/2,height/2-45,1,1);
  gameOver.addImage(gameoverImg);
  gameOver.scale = 0.08;
  
  //creates missile,obstacle,fuel groups
...
function draw() {
  ...
  if (gameState === PLAY){
      //calls obstacle and fuelStation function
      obstacle();
      fuelStation();
      ...
      //moves spaceShip right side when right arrow pressed
      if (keyDown(RIGHT_ARROW)){
          spaceShip.x = spaceShip.x+15;

      }

      //moves spaceShip left side when left arrow pressed
      if (keyDown(LEFT_ARROW)){
          spaceShip.x = spaceShip.x-15;

      }
      
      //calls miniRocket function when space key pressed
      ...
      //destroy missile,asteroid & increase the score
      ...
      //sets the fuelLevel2 position & destroy fuelGroup
      if (fuelGroup.isTouching(spaceShip)){
          fuelGroup.destroyEach();
          fuelLevel2.x = 57;
      }
      
      //destroy asteroid,missile & changes gameState to end if lack of fuel or asteroid hits spaceShip
      if (obstacleGroup.isTouching(spaceShip) || fuelLevel2.x+60 < 0){
          ...
          //caluclates highScore
          if (highScore < score){
              highScore = score;
          }
     
          //changes gameState to play & calls reset function when r key pressed
          if (keyDown("r")){
              gameState = PLAY;
              reset();
          }
